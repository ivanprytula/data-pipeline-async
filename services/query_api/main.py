...
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    ...
    setup_json_logger("query_api")
    logger.info("query_api_starting")
    try:
        async with engine.begin() as conn:
            await conn.execute(text("SELECT 1"))
        logger.info("query_api_db_connected")
    except Exception as exc:
        logger.error("query_api_db_connect_failed", extra={"error": str(exc)})
        raise

    yield

    logger.info("query_api_shutting_down")
