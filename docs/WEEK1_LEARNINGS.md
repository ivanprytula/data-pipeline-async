...
- Database configuration: I discovered that `listen_addresses = '*'` in `postgresql.conf` is required for Docker bridge traffic (host CLI, CI runners) to reach the container — without it, PostgreSQL only accepts connections on its own loopback inside the container. Coupled with auth rules in `pg_hba.conf` that use `scram-sha-256` for Docker bridge connections (`172.16.0.0/12`), this achieves dev/prod parity: you catch auth issues in dev using the same method as production. Loopback connections inside the container use `trust` auth for health checks and convenience.
