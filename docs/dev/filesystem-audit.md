...
| `ai-gateway` | `/tmp`, `/root/.cache/huggingface` (sentence-transformers model cache) | `emptyDir` at `/tmp`; `emptyDir{medium: Memory}` at model cache |
...
- `ai-gateway` model cache uses `medium: Memory` to avoid disk contention; size limited to
...
for svc in ingestor ai-gateway processor dashboard query-api webhook; do
